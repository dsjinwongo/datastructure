...
int main()
{
	...
	check[11] = free_matrix(matrix_t, row, col);
	check[12] = free_matrix(matrix_axt, row, col);
...
int multiply_matrix(int** matrix_a, int** matrix_t, int** matrix_axt, int row, int col)     //행렬의 곱
{
	...
	if (matrix_axt[0][0] == -500)										//post_condition
	{
...
int** create_matrix(int row, int col)                   //행렬 생성과 전처리검사함수
{
	...
	for (i = 0; i < row; i++)
		memset(matrix[i], -500, sizeof(matrix[i]));		/*다른 함수에서 post_condition을 수행하기 위한 함수
														(500은 20*20=400이기 때문에 오류가 발생하기 않기 위해 설정하였다.)*/
